...
optlist, args = getopt.getopt(sys.argv[1:], 'f:d:i:c:s:erp:')
...
if flagsiren=='false':
    ...
    if ext==".csv":
        ...
        for rownum in range(indice,sh.nrows):
            ...
            if sh.row_values(rownum)[col] !="":

                siren=str(int(sh.row_values(rownum)[col])).zfill(9)
        ...
        for rownum in range(indice,sh.nrows):
            ...
            if sh.row_values(rownum)[col] !="":
                ...
                if sh.cell_type(rownum,col) == 2:

                    siren=str(int(sh.row_values(rownum)[col])).zfill(9)

                elif sh.cell_type(rownum,col) ==1:

                    siren=str(sh.row_values(rownum)[col]).replace(' ','').zfill(9)
